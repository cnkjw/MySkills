    ...
    *,
    ...
    payload = {
        ...
        "preferred_final_report_format": [
            ...
            "Selected paper entries: include title, Chinese metadata bullets for 作者, 类别, 发表/更新, 相关度评分, 推荐度评分, 关系类型, then original Abstract, AI-translated Chinese 摘要, relevance reason, recommendation reason, and research ideas",
            "Math formatting: preserve LaTeX math delimiters such as $...$ and $$...$$; never wrap formulas in backticks",
...
def write_markdown_packet(
    ...
    *,
    ...
    lines = [
        ...
        "5. For every selected paper, use a numbered `####` heading. Under it, use exactly these Chinese metadata bullets: `- **作者**：...`, `- **类别**：...`, `- **发表/更新**：...`, `- **相关度评分**：.../100`, `- **推荐度评分**：.../100`, `- **关系类型**：...`. Do not use English metadata labels or semicolon-separated inline metadata.",
        "6. Before `相关理由`, include `**Abstract:** <original summary from this packet>` and `**摘要：** <AI Chinese translation of that Abstract>` in this order.",
        "7. Then include `**相关理由**`, `**推荐理由**`, and `**拓展研究思路**`. Research ideas should be concrete bullets.",
        "8. Preserve LaTeX math delimiters from arXiv text. Keep inline math as `$...$` and display math as `$$...$$`; never wrap LaTeX formulas in backticks.",
        "9. `## 候选论文完整列表（精简版）`: use a compact table with arXiv ID, short title, categories, relevance score, recommendation score, and relation. Do not dump hundreds of unrelated rows; summarize the remaining non-relevant papers in one sentence.",
        "10. `## 方法论注记`: state that candidates came from arXiv API and were evaluated by the executing AI assistant from titles, abstracts, categories, and metadata. Do not claim exhaustive citation coverage.",
        "",
    ]
    if excluded_counts:
